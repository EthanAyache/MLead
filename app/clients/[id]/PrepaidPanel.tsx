...
export default function PrepaidPanel({
  clientId, billingMode, prepaidBalance, avgPrice, topups,
}: {
  clientId: string
  billingMode: 'MONTHLY' | 'PREPAID'
  prepaidBalance: number
  avgPrice: number // prix moyen HT par lead sur les sites du client (0 si aucun)
  topups: Topup[]
}) {
  ...
  async function setFormula(next: 'MONTHLY' | 'PREPAID') {
  ...
  const tabBase = 'px-3.5 h-9 rounded-lg text-sm font-semibold border transition disabled:opacity-50'

  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm mb-6">
      <div className="flex items-center justify-between gap-3 flex-wrap mb-4">
        <h2 className="text-sm font-bold text-gray-900 uppercase tracking-wide">Formule de facturation</h2>
        <div className="flex gap-2">
          <button onClick={() => setFormula('MONTHLY')} disabled={busy} className={`${tabBase} ${mode === 'MONTHLY' ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white border-gray-300 text-gray-700 hover:bg-gray-50'}`}>Mensuel</button>
          <button onClick={() => setFormula('PREPAID')} disabled={busy} className={`${tabBase} ${mode === 'PREPAID' ? 'bg-violet-600 border-violet-600 text-white' : 'bg-white border-gray-300 text-gray-700 hover:bg-gray-50'}`}>Prépayé</button>
        </div>
      </div>

      {mode === 'MONTHLY' ? (
        <p className="text-sm text-gray-500">Facturation classique : une facture Stripe est émise chaque mois selon les leads reçus.</p>
      ) : (
        <>
          <div className="flex items-center justify-between gap-3 flex-wrap bg-violet-50 border border-violet-100 rounded-lg px-4 py-3 mb-4">
            <div>
              <div className="text-xs font-semibold text-violet-700 uppercase">Solde prépayé (HT)</div>
            ...
            <div className="text-xs text-violet-700/80 text-right">
              {avgPrice > 0 ? <>≈ {Math.floor(prepaidBalance / avgPrice)} lead(s) restant(s)<br />(prix moyen {formatEuros(avgPrice)}/lead)</> : 'Aucun site payant configuré'}
              <br />Quand le solde ne couvre plus un lead, l&apos;envoi s&apos;arrête et le client est prévenu.
            </div>
          </div>
