      ...
      <main className="min-h-screen bg-gray-50 p-8">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-2">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Campagnes (réception de leads)</h1>
              <p className="text-gray-500 text-sm mt-1">
                {dossiers.length} campagne{dossiers.length > 1 ? 's' : ''} chez {grouped.length} client{grouped.length > 1 ? 's' : ''}
              </p>
            </div>
          ...
          <p className="text-xs text-gray-400 mb-6">
            Une <strong>campagne</strong> = une source/un site d&apos;un client (avec son lien API et son prix par lead). Un même client peut avoir plusieurs campagnes.
          </p>

          {clients.length === 0 && (
            <div className="bg-amber-50 border border-amber-200 text-amber-800 rounded-xl p-4 mb-4 text-sm">
              Aucun client pour le moment. Crée d&apos;abord un client dans <Link href="/clients" className="font-semibold underline">Clients</Link>, puis reviens créer sa campagne.
            </div>
          )}

          {dossiers.length === 0 ? (
            <div className="bg-white rounded-xl border border-gray-200 p-12 text-center text-gray-500 shadow-sm">
              Aucune campagne. Crée une campagne pour générer un lien API et commencer à recevoir des leads.
            </div>
          ) : (
                ...
                <div key={clientId} className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                  ...
                  <div className="flex items-center gap-2.5 px-4 py-3 bg-gray-50 border-b border-gray-200">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-500">{g.dossiers.length} campagne{g.dossiers.length > 1 ? 's' : ''}</div>
                    </div>
                  </div>
                  {/* Dossiers du client */}
                  <table className="w-full text-sm">
                    <thead className="border-b border-gray-100">
                      <tr>
                        <th className="text-left px-4 py-2 font-semibold text-gray-500 text-xs uppercase">Campagne</th>
