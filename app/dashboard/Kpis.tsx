...
export default async function Kpis() {
  ...
  const [invoices, leads, clients] = await Promise.all([
    prisma.invoice.findMany({ where: { archived: false, ...filter } }),
    prisma.lead.findMany({ where: filter }),
    prisma.client.findMany({ where: { archived: false, ...filter } }),
  ])
  ...
  const kpis: {
    ...
    {
      label: 'Total dû', value: fmtEUR(totalOwed),
    ...
    {
      label: 'Encaissé ce mois', value: fmtEUR(collected),
      sub: trend === null
        ? <span className="text-[#787C8A]">Premier mois suivi</span>
        : <span className={trend >= 0 ? 'text-[#1F8A53] font-semibold' : 'text-[#D23B3B] font-semibold'}>{trend >= 0 ? '▲' : '▼'} {Math.abs(trend)} % vs mois dernier</span>,
    ...
    {
      label: 'En retard', value: fmtEUR(lateAmount), valueClass: 'text-[#D23B3B]',
    ...
    {
      label: 'Leads vendus', value: totalLeads.toLocaleString('fr-FR'),
    ...
    {
      label: 'Clients actifs', value: totalClients.toString(),
