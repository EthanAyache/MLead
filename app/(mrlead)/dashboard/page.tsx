...
export default async function DashboardPage() {
  ...
  const [clients, apporteurs, aAppelerCount, monthlyInvoices, monthBillableLeads] = await Promise.all([
    prisma.client.findMany({ where: { archived: false, ...filter }, orderBy: { name: 'asc' }, include: { apporteur: { select: { name: true } }, categories: { select: { id: true, name: true, position: true }, orderBy: { position: 'asc' } } } }),
      ...
      <main className="max-w-[1320px] mx-auto px-[22px] py-[22px]">
        <div className="flex items-start gap-4 flex-wrap mb-[18px] mt-2">
          ...
          <div className="ml-auto flex gap-2.5 flex-wrap">
            ...
            {me?.role === 'ADMIN' && (
              <Link href="/admin/sites" className="h-[42px] px-4 rounded-[11px] bg-white border border-[#DCDDE6] text-[#16171D] font-semibold text-sm hover:bg-[#FAFAFC] transition flex items-center gap-2">
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <Link href="/departements" className="h-[42px] px-4 rounded-[11px] bg-white border border-[#DCDDE6] text-[#16171D] font-semibold text-sm hover:bg-[#FAFAFC] transition flex items-center gap-2">
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
